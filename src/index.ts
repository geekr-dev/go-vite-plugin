...
interface GoPlugin extends Plugin {
 ...
 */
export default function go(config: string | string[] | PluginConfig): [GoPlugin, ...Plugin[]] {
    const pluginConfig = resolvePluginConfig(config)

    return [
        resolveGoPlugin(pluginConfig),
        ...resolveFullReloadConfig(pluginConfig) as Plugin[],
    ];
}

/**
 * Resolve the Go Plugin configuration.
 */
function resolveGoPlugin(pluginConfig: Required<PluginConfig>): GoPlugin {
    ...
    return {
        ...
        config: (userConfig, { command, mode }) => {
            ...
            return {
                ...
                server: {
                    origin: userConfig.server?.origin ?? '__go_vite_placeholder__',
                    ...(process.env.DOCKER ? {
        ...
        transform(code) {
            if (resolvedConfig.command === 'serve') {
                code = code.replace(/__go_vite_placeholder__/g, viteDevServerUrl)
        ...
        configureServer(server) {
            ...
            server.httpServer?.once('listening', () => {
                ...
                if (isAddressInfo(address)) {
                    ...
                    setTimeout(() => {
                        server.config.logger.info(`\n  ${colors.red(`${colors.bold('GO')} ${goVersion()}`)}  ${colors.dim('plugin')} ${colors.bold(`v${pluginVersion()}`)}`)
                        server.config.logger.info('')
...
function ensureCommandShouldRunInEnvironment(command: 'build' | 'serve', env: Record<string, string>): void {
    if (command === 'build' || env.GO_BYPASS_ENV_CHECK === '1') {
        return;
    }
...
/**
 * The version of the Go Vite plugin being run.
 */
...
function resolvePluginConfig(config: string | string[] | PluginConfig): Required<PluginConfig> {
    if (typeof config === 'undefined') {
        throw new Error('go-vite-plugin: missing configuration.')
    }
    ...
    if (typeof config.input === 'undefined') {
        throw new Error('go-vite-plugin: missing configuration for "input".')
    }
    ...
    if (typeof config.publicDirectory === 'string') {
        ...
        if (config.publicDirectory === '') {
            throw new Error('go-vite-plugin: publicDirectory must be a subdirectory. E.g. \'public\'.')
        }
    }
    ...
    if (typeof config.buildDirectory === 'string') {
        ...
        if (config.buildDirectory === '') {
            throw new Error('go-vite-plugin: buildDirectory must be a subdirectory. E.g. \'build\'.')
        }
    }
...
function noExternalInertiaHelpers(config: UserConfig): true | Array<string | RegExp> {
    ...
    const pluginNoExternal = ['go-vite-plugin']
